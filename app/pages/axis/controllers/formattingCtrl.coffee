angular.module('app').controller 'AxisFormattingCtrl', ($log, $scope, $templateCache, $state) ->
  ...
  $scope.options.formatObj.tickFormat = (val) -> '>' + val
  ...
  d3.csv("data/pages/#{menu.url}/data#{$state.current.url}.csv", (rows) ->
    ...
    $scope.options.formatObj = {tickValues:_.pluck(rows,'from').slice(1)}
    $scope.$apply()
  )
